...
var GooCompleter = new Class({
	...
	initialize: function (field, options) {
		...
		if (this.options.use_listbox)
		{
			...
			this.listbox.addEvent('click:relay(li)', function(event, target) {
				...
				$(this.options.GooValue).set("value", target.getProperty("value"));
			}.bind(this));
			...
			//    this.listbox.setStyle('display', 'none');
			//    this.typebox.setStyle('display', 'none');
			//    var target = this.listbox.getElement(".selected");
			//    if (target) {
			//        this.field.set('value', this.normalizeValue(target.get('html')));
			//        if (this.options.use_typebox)
			//            this.typebox.empty();
			...
			//        this.listbox.setStyle('display', 'none');
			//        $(this.options.GooValue).set("value", target.get("value"));
			//    }
			//    if (this.field.value == '') {
			//        $(this.options.GooValue).set("value", "");
			//    }
	...
	*/
	showSuggestions: function(suggestions)
	{
		...
		if (suggestions.length > 0)
		{
			...
			if (this.options.use_typebox)						
				this.writeTypebox(suggestions[0].name);			
		
				
			Object.each(suggestions, function(value) {
				
				var html_value = value.name;
				var html_name = value.name;
				...
				{	
					...
					if (this.options.hightlight)
					{
					    ...
					    html_value = html_name.substr(0, this.field.get('value').length) + '<span class="goocompleter_hightlight">';
					    html_value = html_value + html_name.substr(this.field.get('value').length) + '</span>';
	...
	*/
	writeTypebox: function (suggestion) {
		...
		replacement = suggestion && suggestion.name ? suggestion.name.substr(this.field.get('value').length) : suggestion.substr(this.field.get('value').length);
	...
	*/
	searchCache: function (search) {
		...
		Object.each(this.suggestions, function(value) {
			...
			if (search.toLowerCase() == value.name.substr(0, search.length).toLowerCase())								
	...
	*/
	...
	setRelPosition2: function () {
	    ...
	    var y_offset = this.options.typebox_offset.x, x_offset=this.options.typebox_offset.y;
