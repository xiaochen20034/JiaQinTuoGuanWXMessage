...
var VMFormValidator = new Class({
	...
	options: {
		...
		errorDisplay: 'aboveForm',				//Where the errors are displayed: 'aboveInput', 'belowInput', 'aboveForm',element,alert
		...
		errorElement: 'error',			//Class for the input box when an error is detected 
		errorLabel: 'errorLabel',				//Class for the input label when an error is detected
		errorLabelClass: 'error',      //Class for the error message when an error is detected
		labelDisplay: '.hold .msg',					//Where the label is in relation to the form element in the HTML: 'before', 'after'
		...
		validateOnBlur: false,					//Validate each form element on blur, defaults true
	...
	buildFormElementArray: function(form, formElements, validator){
		var formElementsArray = form.getElements('input,textarea,select').clean();
	...
	executeValidators: function (elementName, event, eventType) {
	    ...
	    Object.each(this.formElements[elementName]['validators'], function (index) {
	        var fun = index.validatorFunction;
	        var x;
	        if (typeof (fun) == 'function') {
	            x = fun.call();
	        } else {
	             x= eval(fun); //Is there any way NOT to use eval here? Let me know
	        }
		    if (x==false) {
		        return false;
		    }
	...
	executeAllValidators: function (e) {
	    ...
	    //    this.executeValidators(this.formElements[i].elName, e, 'submit');
	    //    if (this.validateErrorCount != 0 && !this.options.validateElementMultiple) {
	    //        return;
	    //    }
	    //}
	...
	createErrorId: function(name, error){ 
	    ...
	    var errorId = name + "ErrorListItem";//errorString;
	...
	inputSuccess: function(element, label){ 
		...
		if (label&&label.hasClass(this.options.errorLabel)) { label.removeClass(this.options.errorLabel); }

		if(label){label.addClass(this.options.successLabelClass);}
	...
	success: function(name, element, label, error){
		 ...
		 if(this.options.errorDisplay == 'element'){
			if ($(name+'ErrorList') != null) {
				var errorList = $(name+'ErrorList');
			} else {
			    var errorList = element.getParent(this.options.labelDisplay);
			    errorList.set('html','');
			}
			errorList.removeClass(this.options.errorLabelClass);
			errorList.addClass(this.options.successLabelClass);


			
			element.removeClass(this.options.errorElement);
			element.addClass(this.options.successElementClass);
	...
	error: function (name, element, label, error) {
		...
		if (label&&label.hasClass(this.options.successLabelClass)) { label.removeClass(this.options.successLabelClass); }
		
		if(label){label.addClass(this.options.errorLabel);}
		element.addClass(this.options.errorElement);
		
		if (this.options.errorDisplay == 'aboveForm'){
			if ($(this.form + 'ErrorList')) { 
				var errorList = $(this.form + 'ErrorList');
			} else if (this.form.getParent().getElement(this.options.labelDisplay)) {
			    var errorList = this.createErrorList(this.form);
			    errorList.inject(this.form.getParent().getElement(this.options.labelDisplay));
			}else {
				var errorList = this.createErrorList(this.form);
			...
			if ($(name+'ErrorList') != null) { 
				...
				if (this.options.labelDisplay == 'before'){
					errorList.inject(label?label:element, 'before');
				} else if (this.options.labelDisplay == 'after'){
					errorList.inject(element, 'before');
				}else if(this.options.labelDisplay == 'none'){
					//不操作
				}
			}
			...
			if ($(name+'ErrorList') != null) {
				...
				if (this.options.labelDisplay == 'before'){
					...
					errorList.inject(label?label:element, 'after');
				}else if(this.options.labelDisplay == 'none'){
					//不操作
				}
			}
		}else if(this.options.errorDisplay == 'element'){
			if ($(name+'ErrorList') != null) {
				var errorList = $(name+'ErrorList');
			} else {
			    var errorList = element.getParent(this.options.labelDisplay);
			    errorList.set('html',error);
			}
			if (errorList.hasClass(this.options.successLabelClass)) {
			    errorList.removeClass(this.options.successLabelClass);
			}
			errorList.addClass(this.options.errorLabelClass);
			
			return;
		}else if(this.options.errorDisplay == 'none'){
			//不操作
		} else if (this.options.errorDisplay == 'alert') {
		    alert(error);
		    return;
		}
	...
	internalNumber: function (name, error, type, num1, num2) {
	    var element = $(this.form).getElement('[name=' + name + ']');
	    var label = $(this.form).getElement('label[for=' + name + ']');
	    var value = element.get('value').trim();
	    var valid = true;
	...
	rangeLength: function (name, minLength,maxLength, error) {
	    if (!error) { error = 'This field may contain no more than ' + maxLength + ' characters.'; }
	    var vFunction = "this.internalLength('" + name + "', '" + error + "', 'range', " + minLength + "," + maxLength + ");";
	    this.setValidator(name, 'rangeLength', vFunction, error);
	},
	...
	range: function(name, minLength, maxLength, error){
		...
		var vFunction = "this.internalNumber('" + name + "', '" + error + "', 'range', " + minLength + ", " + maxLength + ");";
		this.setValidator(name, 'range', vFunction, error);
	},
	min: function (name, min, error) {
	    if (!error) { error = 'This field must contain at least ' + min + ' characters.'; }
	    var vFunction = "this.internalNumber('" + name + "', '" + error + "', 'minRange', " + min + ",null);";
	    this.setValidator(name, 'range', vFunction, error);
	},
	max: function (name,  max, error) {
	    if (!error) { error = 'This field must contain at least ' + max + ' and no more than ' + maxLength + ' characters.'; }
	    var vFunction = "this.internalNumber('" + name + "', '" + error + "', 'maxRange', null," + max + ");";
	    this.setValidator(name, 'range', vFunction, error);
	},
	...
	commonChar: function(name, error){
		if (!error){ error = "只能输入字母、数字、下划线."; }
		this.validateRegEx('^[a-zA-Z0-9_]+$', name, error);
	},
